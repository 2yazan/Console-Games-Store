...
export default function CartQuantity({ cart, isDisabled, setIsDisabled }) {
  ...
  const handleAdd = () => {
    if (quantity < cart.game_owner.stocks) {
  ...
  const handleQuantityChange = (e) => {
    if (e.target.value > cart.game_owner.stocks) {
      setQuantity(cart.game_owner.stocks);
    } else {
  ...
  return (
    <div className="flex items-center pt-3">
      <button
        onClick={handleMinus}
        disabled={isDisabled}
        className="flex justify-center w-8 h-8 items-center focus:outline-none text-white bg-green-700	 border border-transparent enabled:hover:bg-cyan-800 focus:ring-cyan-300 dark:bg-cyan-600 dark:enabled:hover:bg-green-700	 dark:focus:ring-cyan-800 focus:ring-2 rounded-l-md disabled:opacity-50 disabled:hover:cursor-not-allowed"
      >
        <Minus />
      </button>
      <input
      ...
      <button
        onClick={handleAdd}
        disabled={isDisabled}
        className="flex justify-center w-8 h-8 items-center focus:outline-none text-white bg-green-700	 border border-transparent enabled:hover:bg-cyan-800 focus:ring-cyan-300 dark:bg-cyan-600 dark:enabled:hover:bg-green-700	 dark:focus:ring-cyan-800 focus:ring-2 rounded-r-md disabled:opacity-50 disabled:hover:cursor-not-allowed"
      >
        <Plus />
      </button>
      <button
